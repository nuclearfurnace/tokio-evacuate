...
pub fn ms(num: u64) -> Duration { Duration::from_millis(num) }
...
impl IntoTimeout for Option<Duration> {
    fn into_timeout(self) -> Self { self }
}

impl IntoTimeout for Duration {
    fn into_timeout(self) -> Option<Duration> { Some(self) }
}

/// Turn the timer state once
pub fn turn<T: IntoTimeout>(timer: &mut Timer<MockPark>, duration: T) { timer.turn(duration.into_timeout()).unwrap(); }
    ...
    F: FnOnce(&mut Timer<MockPark>, &mut MockTime) -> R,
{
    ...
    ::tokio_timer::clock::with_default(&now, &mut enter, |enter| {
        ...
        ::tokio_timer::with_default(&handle, enter, |_| {
            lazy(|| Ok::<_, ()>(f(&mut timer, &mut time))).wait().unwrap()
        })
    })
}
...
impl MockTime {
    ...
    pub fn mock_park(&self) -> MockPark {
        let inner = self.inner.clone();
        MockPark { inner, _p: PhantomData }
    }

    pub fn now(&self) -> Instant { self.inner.lock().unwrap().now() }

    /// Returns the total amount of time the time has been advanced.
    pub fn advanced(&self) -> Duration { self.inner.lock().unwrap().advance }
    ...
    pub fn park_for(&self, duration: Duration) { self.inner.lock().unwrap().park_for = Some(duration); }
}
...
impl Now for MockNow {
    fn now(&self) -> Instant { self.inner.lock().unwrap().now() }
}

impl State {
    fn now(&self) -> Instant { self.base + self.advance }

    fn advance(&mut self, duration: Duration) { self.advance += duration; }
}
